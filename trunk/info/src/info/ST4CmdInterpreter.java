...
public class ST4CmdInterpreter<TContext,TCmdResult> extends AbstractMap<String,TCmdResult>
{
    ...
    /** A short string, which preceeds all messages to the application log. Used for
     ...
     *   @param logContext
     * A string, which preceeds all later logged messages (debugging only). Pass null if
     * not such context string is needed.
     */
     ...
     * template iterates through the map. Since the command interpreter logically seen
    ...
    {
        errCnt_.warning();
        _logger.warn( logContext_ + "A map iteration has been tried on this"
                      + " field of the data model. This is possible as the field formally"
    ...
    } /* End of ST4CmdInterpreter.containsKey */
